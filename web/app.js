...
const profileInvitesButton = document.querySelector("#profile-invites-button");
...
const invitesView = document.querySelector("#invites-view");
...
const ALL_VIEWS = [homeView, profileView, invitesView, missionsView, clansView, leagueView, liveView, esportView];
...
function showView(view, options = {}) {
  ...
  ALL_VIEWS.forEach(v => v && v.classList.toggle("is-active", v === view));
...
function isCaptainOfMatch(m) {
  ...
  return (m.team1?.id && state.myLeagueTeamIds.includes(m.team1.id)) ||
         (m.team2?.id && state.myLeagueTeamIds.includes(m.team2.id));
}
function renderLiveBracket(data) {
  if (!data.bracket) {
    liveContent.innerHTML = `<div class="live-reg-state"><div class="live-reg-icon">📋</div><h3>Идёт регистрация</h3><p>Сетка будет сгенерирована в понедельник в 04:00</p><div class="live-reg-count">${data.registeredTeams || 0} команд зарегистрировано</div></div>`;
    return;
  }
  ...
  let html = `<div class="live-stats-bar"><div><strong>${b.registeredTeams || 0}</strong><small>Команд</small></div><div><strong>${b.playedMatches || 0}/${b.totalMatches || 0}</strong><small>Матчей</small></div><div><strong>${total}</strong><small>Раундов</small></div></div>`;
  ...
  roundNums.forEach(rnd => {
    ...
    (rounds[rnd] || []).forEach(m => { html += renderLiveMatch(m); });
    html += `</div>`;
  });
...
function renderLiveMatch(m) {
  ...
  if (m.status === "pending" && t1 && t2) {
    if (m.faceitMatchUrl) faceitBtn = `<a href="${escapeHtml(m.faceitMatchUrl)}" target="_blank" class="live-faceit-link">🔗 Открыть комнату FACEIT</a>`;
    else if (isCaptainOfMatch(m)) faceitBtn = `<button class="live-faceit-btn" type="button" data-match-id="${m.id}">🎮 Создать комнату FACEIT</button>`;
  }
  return `<div class="live-match"><div class="${barClass}">${barText}</div><div class="live-team ${t1cls}"><span>${escapeHtml(t1?.name || "TBD")}</span><b>${s1}</b></div><div class="live-divider"></div><div class="live-team ${t2cls}"><span>${escapeHtml(t2?.name || "TBD")}</span><b>${s2}</b></div>${faceitBtn}</div>`;
}
...
async function loadLiveBracket() {
  ...
  try {
    ...
    if (data.bracket?.status === "active") state.liveRefreshTimer = setTimeout(loadLiveBracket, 30000);
  } catch { liveContent.innerHTML = `<div class="live-error">⚠️ Не удалось загрузить.</div>`; }
}
...
async function createFaceitRoom(matchId, btn) {
  ...
  try {
    ...
    if (result.ok && result.faceitMatchUrl) {
      const link = document.createElement("a"); link.href = result.faceitMatchUrl; link.target = "_blank"; link.className = "live-faceit-link"; link.textContent = "🔗 Открыть комнату FACEIT";
      btn.replaceWith(link); tg?.HapticFeedback?.notificationOccurred("success"); setStatus("Комната создана!");
    } else if (result.fallback) { btn.textContent = "🎮 Создать вручную"; btn.disabled = false; if (result.faceitHubUrl && tg?.openLink) tg.openLink(result.faceitHubUrl);
    } else if (result.already) { const link = document.createElement("a"); link.href = result.faceitMatchUrl; link.target = "_blank"; link.className = "live-faceit-link"; link.textContent = "🔗 Открыть комнату FACEIT"; btn.replaceWith(link); }
  } catch (err) { btn.disabled = false; btn.textContent = "🎮 Создать комнату FACEIT"; setStatus(errorMessage(err, "Не удалось."));  }
}

// ── RENDER ──
function renderTop(items) {
  if (!topListNode) return;
  ...
  topListNode.innerHTML = items.map((item, i) => {
    ...
    return `<li><strong>${i + 1}. ${escapeHtml(name)}${rank}</strong><span>${item.total}</span></li>`;
  }).join("");
}
function renderRank(user) {
  ...
  if (rankTitleNode) rankTitleNode.textContent = `${rank.icon || "🥉"} ${rank.title || "Recruit"}`;
  if (rankScoreNode) rankScoreNode.textContent = String(score);
  if (rankMeterFillNode) rankMeterFillNode.style.width = `${pct}%`;
  if (rankNextNode) rankNextNode.textContent = rank.isMax ? "Максимальный ранг." : `До ${rank.nextIcon || ""} ${rank.nextTitle || ""}: ${rank.remaining || 0}`;
  if (rankInvitesNode) rankInvitesNode.textContent = String(progress.invites || 0);
  if (rankCoinsNode) rankCoinsNode.textContent = String(progress.coins || 0);
  if (rankVisitsNode) rankVisitsNode.textContent = String(progress.visits || 0);
  if (headerCoinBalance) headerCoinBalance.textContent = String(progress.coins || 0);
  if (coinNoteNode) coinNoteNode.textContent = `За каждого приглашённого: ${progress.inviteCoinReward || 20} монет.`;
  if (rankHintText) rankHintText.textContent = rank.isMax ? "Максимальный ранг." : `Пригласи ещё ${rank.remaining || 0} и стань ${rank.nextTitle || ""}.`;
  ...
  const p = document.querySelector("#home-missions-progress");
  if (p) p.style.width = `${Math.min(100, (invites / 50) * 100)}%`;
}
function renderMissions(missions) {
  ...
  if (dailyRewardButton) { dailyRewardButton.disabled = !daily.canClaim; dailyRewardButton.textContent = daily.canClaim ? "🪙 Забрать награду" : "✓ Забрано · Завтра снова"; }
  if (visitRewardButton) { visitRewardButton.disabled = !visit.canClaim; visitRewardButton.textContent = visit.canClaim ? "Забрать" : "Готово"; }
  ...
  streakMilestones.forEach(m => m.classList.toggle("is-reached", days >= Number(m.dataset.streak || 0)));
  ...
  const r = document.querySelector("#home-missions-reward");
  if (r) r.textContent = daily.canClaim ? "🔥 Daily Reward ждёт!" : visit.canClaim ? "Визит дня доступен" : "Всё забрано на сегодня";
}
...
function renderClanTopWidget(items) {
  ...
  clanTopListWidget.innerHTML = items.slice(0, 3).map((c, i) =>
    `<div class="clan-top-row"><div class="clan-top-place ${places[i] || ''}">${i + 1}</div><div class="clan-top-avatar">${c.name.slice(0, 2).toUpperCase()}</div><div class="clan-top-info"><h5>${escapeHtml(c.name)}</h5><p>LVL ${c.level?.level || 1} · ${c.membersCount || 0} уч.</p></div><div class="clan-top-xp">${formatXp(c.periodXp || c.weeklyXp)}</div></div>`
  ).join("");
}
...
function renderLeague(data) {
  ...
  if (leagueRoleNode) leagueRoleNode.textContent = user.isAdmin ? "Админ" : user.role === "captain" ? "Капитан" : "Игрок";
  if (leagueXpNode) leagueXpNode.textContent = `${Number(user.xp || 0)} XP`;
  if (leagueRegisterFaceit) leagueRegisterFaceit.value = user.faceitNickname || "";
  document.body.classList.toggle("has-league-admin", Boolean(user.isAdmin));
  if (faceitHubBanner) { faceitHubBanner.hidden = !user.faceitHubUrl; if (user.faceitHubUrl && faceitHubLink) faceitHubLink.href = user.faceitHubUrl; }
  state.myLeagueTeamIds = (state.league.myTeams || []).filter(t => t.status === "active").map(t => Number(t.id));
...
function renderLeagueNextStep(user, teams) {
  if (!leagueNextTitle) return;
  const hasFaceit = Boolean((user.faceitNickname || "").trim());
  const hasTeam = Boolean(teams.length);
  if (!hasFaceit) { leagueNextTitle.textContent = "Укажи FACEIT nickname"; leagueNextNote.textContent = "Матчи турнира проходят через FACEIT Hub."; leagueNextButton.textContent = "Указать"; leagueNextButton.dataset.target = "register"; return; }
  if (!hasTeam) { leagueNextTitle.textContent = "Вступи или создай команду"; leagueNextNote.textContent = "Есть код капитана? Введи его. Хочешь собрать состав — создай заявку."; leagueNextButton.textContent = "Открыть заявку"; leagueNextButton.dataset.target = "register"; return; }
  leagueNextTitle.textContent = "Команда готова"; leagueNextNote.textContent = "Проверь состав, пригласи игроков или смотри таблицу."; leagueNextButton.textContent = "Таблица"; leagueNextButton.dataset.target = "table";
}
function renderLeagueJoinAccess(user) {
  if (!leagueJoinForm) return;
  const hasFaceit = Boolean((user.faceitNickname || "").trim());
  leagueJoinForm.hidden = !hasFaceit;
  if (leagueJoinNote) leagueJoinNote.textContent = hasFaceit ? "Введи код капитана." : "Сначала укажи FACEIT nickname.";
}
...
function selectSeriesFormat(format) {
  ...
  if (leagueTeamFormat) leagueTeamFormat.value = state.selectedLeagueFormat;
  const isDuo = state.selectedLeagueFormat === "2x2";
  if (seriesSelectedTitle) seriesSelectedTitle.textContent = isDuo ? "Duo Strike" : "Full Squad Challenge";
  if (seriesSelectedBadge) seriesSelectedBadge.textContent = isDuo ? "⚡ 2×2" : "🛡️ 5×5";
...
function renderLeagueSeasons(seasons) {
  if (!leagueSeasonsNode) return;
  if (!seasons.length) { leagueSeasonsNode.innerHTML = "<span><strong>Скоро</strong></span>"; return; }
  leagueSeasonsNode.innerHTML = seasons.map(s => `<span><strong>${escapeHtml(s.formatTitle || s.gameTitle)}</strong><small>${escapeHtml(leagueStatusLabel(s.status))}</small></span>`).join("");
}
function renderLeagueMyTeam(teams) {
  if (!leagueMyTeamNode) return;
...
function renderLeagueTeams(teams) {
  ...
  leagueTeamsList.innerHTML = teams.map(t => `<article class="league-row"><div><strong>${escapeHtml(t.name)}</strong><span>${escapeHtml(t.formatTitle || t.gameTitle)} · ${escapeHtml(leagueStatusLabel(t.status))}</span></div><small>${t.members?.length || 0}</small></article>`).join("");
}
function renderLeagueStandings() {
  const tabs = state.league?.standingTabs || [];
  if (!tabs.length || !leagueStandingsTabs) return;
  ...
  leagueStandingsTabs.innerHTML = tabs.map(t => `<button type="button" data-standing-tab="${escapeHtml(t.key)}" class="${t.key === state.leagueStandingTab ? "is-current" : ""}">${escapeHtml(t.title)}</button>`).join("");
  const active = tabs.find(t => t.key === state.leagueStandingTab) || tabs[0];
  const season = active.season;
  if (!season) { if (leagueStandingsMeta) leagueStandingsMeta.innerHTML = ""; if (leagueStandingsList) leagueStandingsList.innerHTML = ""; return; }
  if (leagueStandingsMeta) leagueStandingsMeta.innerHTML = `<strong>${escapeHtml(season.name || active.title)}</strong><span>${escapeHtml(leagueStatusLabel(season.status))} · команд: ${season.teamsRegistered || 0}</span>`;
  const items = active.standings || [];
  if (!leagueStandingsList) return;
...
function renderLeaguePending(items, isAdmin) {
  if (!leagueAdminPanel) return;
  leagueAdminPanel.hidden = !isAdmin; if (leagueAdminEmpty) leagueAdminEmpty.hidden = isAdmin;
  if (!isAdmin || !leaguePendingList) return;
  if (!items.length) { leaguePendingList.innerHTML = "<p class=\"panel-note\">Нет заявок.</p>"; return; }
  leaguePendingList.innerHTML = items.map(t => `<article class="league-row league-admin-row"><div><strong>${escapeHtml(t.name)}</strong><span>${escapeHtml(t.formatTitle || t.gameTitle)} · ${escapeHtml(t.captainName || "-")}</span></div><div class="league-admin-actions"><button type="button" data-team-id="${t.id}" data-status="active">Принять</button><button type="button" data-team-id="${t.id}" data-status="rejected">Отклонить</button></div></article>`).join("");
}
async function leagueRegister() {
  if (!leagueRegisterButton) return; leagueRegisterButton.disabled = true;
  const faceitNick = leagueRegisterFaceit?.value.trim() || "";
  if (faceitNick && faceitVerifyStatus) {
    ...
    } catch { if (faceitVerifyStatus) faceitVerifyStatus.textContent = "⚠️ Не удалось проверить"; }
  }
  try { const result = await postApi("/api/league/register", { game: "cs2", nick: faceitNick, faceitNick }); renderLeague(result.league || {}); setStatus("Данные сохранены."); }
  catch { setStatus("Не удалось сохранить."); }
  finally { leagueRegisterButton.disabled = false; }
}
async function leagueCreateTeam() {
  if (!leagueCreateTeamButton) return; leagueCreateTeamButton.disabled = true;
  try { const result = await postApi("/api/league/team", { game: "cs2", format: leagueTeamFormat.value, name: leagueTeamName.value.trim() }); if (leagueTeamName) leagueTeamName.value = ""; renderLeague(result.league || {}); setStatus("Заявка отправлена."); }
  catch { setStatus("Не удалось."); }
  finally { syncLeagueTeamFormat(); }
}
function syncLeagueTeamFormat() {
  if (leagueTeamFormatWrap) leagueTeamFormatWrap.hidden = false;
  if (leagueCreateTeamButton) leagueCreateTeamButton.disabled = !leagueTeamFormat?.value;
}
async function leagueUpdateTeam(teamId, status) {
  try { const result = await postApi("/api/league/team/status", { teamId, status }); renderLeague(result.league || {}); setStatus(status === "active" ? "Принята." : "Отклонена."); }
  catch { setStatus("Не удалось."); }
}
function openLeagueInvite(teamId) {
  const team = (state.league?.myTeams || []).find(t => Number(t.id) === Number(teamId));
  if (!team || !leagueInviteSheet) return;
  if (leagueInviteCodeNode) leagueInviteCodeNode.textContent = team.inviteToken;
  if (leagueCopyCodeButton) leagueCopyCodeButton.textContent = "Скопировать";
  leagueInviteSheet.hidden = false;
}
function closeLeagueInvite() { if (leagueInviteSheet) leagueInviteSheet.hidden = true; }
async function copyLeagueInviteCode() {
  await navigator.clipboard.writeText(leagueInviteCodeNode?.textContent.trim() || "");
  if (leagueCopyCodeButton) { leagueCopyCodeButton.textContent = "Скопировано ✓"; setTimeout(() => { leagueCopyCodeButton.textContent = "Скопировать"; }, 2000); }
}
function shareLeagueInviteCode() {
  const code = leagueInviteCodeNode?.textContent.trim() || "";
  const url = `https://t.me/share/url?url=&text=${encodeURIComponent(`Вступай в команду HardPoint Series!\nКод: ${code}`)}`;
  if (tg?.openTelegramLink) tg.openTelegramLink(url); else window.open(url, "_blank");
}
async function leagueJoinTeam() {
  if (!leagueJoinButton) return; leagueJoinButton.disabled = true; leagueJoinButton.textContent = "Проверяем...";
  try { const result = await postApi("/api/league/team/join", { invite_code: leagueJoinCodeInput?.value.trim() || "" }); if (leagueJoinCodeInput) leagueJoinCodeInput.value = ""; renderLeague(result.league || {}); setStatus(`Ты в команде ${result.team?.name || ""}!`); showLeagueSection("overview"); }
  catch (err) { if (leagueJoinNote) leagueJoinNote.textContent = errorMessage(err, "Не удалось."); }
...
async function load() {
  try {
    ...
    if (inviteLinkNode) inviteLinkNode.textContent = me.inviteLink;
    if (weeklyCountNode) weeklyCountNode.textContent = me.weeklyInvitedCount;
    if (allTimeCountNode) allTimeCountNode.textContent = me.allTimeInvitedCount;
    if (clubLoginInput) clubLoginInput.value = me.user.clubLogin || "";
    const displayName = me.user.username ? `@${me.user.username}` : (me.user.name || "Игрок");
    if (homeUsernameNode) homeUsernameNode.textContent = displayName;
    if (homeAvatarButton) homeAvatarButton.textContent = initialsFor(me.user.name || me.user.username || "HP");
    renderRank(me.user); renderMissions(me.missions || {}); renderClans(clans); renderLeague(league);
    showWeeklyTop();
  } catch {
    if (inviteLinkNode) inviteLinkNode.textContent = "Откройте из Telegram.";
    if (homeUsernameNode) homeUsernameNode.textContent = "@hardpoint";
    if (homeAvatarButton) homeAvatarButton.textContent = "HP";
    renderRank({ progress: {}, rank: {} }); renderMissions({}); renderClans({}); renderLeague({});
...
async function claimMission(mission) {
  ...
  } catch { tg?.HapticFeedback?.notificationOccurred("error"); setStatus("Недоступно."); }
  finally { if (button.textContent !== "✓ Забрано · Завтра снова" && button.textContent !== "Получено" && button.textContent !== "На проверке") button.disabled = false; }
}
function activateTab(active) {
  [weeklyTab, allTimeTab].forEach(b => { if (!b) return; const on = b === active; b.classList.toggle("is-active", on); b.setAttribute("aria-selected", String(on)); b.style.color = on ? "#171714" : "var(--muted)"; b.style.background = on ? "var(--yellow)" : "transparent"; });
}
function showWeeklyTop() { activateTab(weeklyTab); if (topTitleNode) topTitleNode.textContent = "Топ недели"; renderTop(state.weeklyTop); }
function showAllTimeTop() { activateTab(allTimeTab); if (topTitleNode) topTitleNode.textContent = "Топ за всё время"; renderTop(state.allTimeTop); }
function activateClanTab(active) {
  [clanWeeklyTab, clanMonthlyTab].forEach(b => { if (!b) return; const on = b === active; b.classList.toggle("is-active", on); b.setAttribute("aria-selected", String(on)); b.style.color = on ? "#171714" : "var(--muted)"; b.style.background = on ? "var(--yellow)" : "transparent"; });
}
...
// ── EVENTS ──
copyButton?.addEventListener("click", async () => { if (!state.inviteLink) return; await navigator.clipboard.writeText(state.inviteLink); tg?.HapticFeedback?.notificationOccurred("success"); setStatus("Ссылка скопирована."); });
shareButton?.addEventListener("click", () => { if (!state.inviteLink) return; tg?.openTelegramLink(`https://t.me/share/url?url=${encodeURIComponent(state.inviteLink)}&text=${encodeURIComponent("Заходи в HardPoint")}`); });
saveLoginButton?.addEventListener("click", async () => { saveLoginButton.disabled = true; try { const r = await postApi("/api/club-login", { clubLogin: clubLoginInput.value.trim() }); clubLoginInput.value = r.clubLogin || ""; setStatus("Логин сохранён."); } catch { setStatus("Не удалось."); } finally { saveLoginButton.disabled = false; } });
...
profileInvitesButton?.addEventListener("click", () => showView(invitesView));
...
document.addEventListener("click", e => { if (!profileDropdown.contains(e.target) && !profileMenuButton.contains(e.target)) closeProfileDropdown(); });
...
liveContent?.addEventListener("click", e => { const btn = e.target.closest("button.live-faceit-btn"); if (!btn) return; createFaceitRoom(btn.dataset.matchId, btn); });
...
leagueStandingsTabs?.addEventListener("click", e => { const b = e.target.closest("button[data-standing-tab]"); if (!b) return; state.leagueStandingTab = b.dataset.standingTab; renderLeagueStandings(); });
leagueMyTeamNode?.addEventListener("click", e => { const b = e.target.closest("button[data-team-id]"); if (!b) return; openLeagueInvite(Number(b.dataset.teamId)); });
...
leaguePendingList?.addEventListener("click", e => { const b = e.target.closest("button[data-team-id]"); if (!b) return; leagueUpdateTeam(Number(b.dataset.teamId), b.dataset.status); });
...
if (publicClansList) publicClansList.addEventListener("click", e => { const b = e.target.closest("button[data-clan-id]"); if (!b) return; joinClan({ clanId: Number(b.dataset.clanId) }); });
...
teamFinalsButton?.addEventListener("click", () => { const open = !teamFinalsPanel.classList.contains("is-open"); teamFinalsPanel.classList.toggle("is-open", open); teamFinalsPanel.setAttribute("aria-hidden", String(!open)); teamFinalsButton.classList.toggle("is-active", open); teamFinalsButton.setAttribute("aria-expanded", String(open)); });
namaIconButton?.addEventListener("click", () => { const open = !namaProfilePanel.classList.contains("is-open"); namaProfilePanel.classList.toggle("is-open", open); namaProfilePanel.setAttribute("aria-hidden", String(!open)); namaIconButton.classList.toggle("is-active", open); namaIconButton.setAttribute("aria-expanded", String(open)); });
